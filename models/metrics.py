...
class MyLoss(nn.Module):
    ...
    def _weight_loss(self, weight, in_features, label):
        weight = F.normalize(weight, dim=1)
        weight_label = weight[label]
        temp_res = weight_label.repeat(1, weight.shape[0])
        temp_res = temp_res.reshape(label.shape[0] * weight.shape[0], in_features)
    ...
    def forward(self, input, label):
        ...
        output *= self.s
        # print(output)
        weight_loss = self._weight_loss(self.weight, self.in_features, label)
        # weight_loss = 0
...
class ArcMarginProduct(nn.Module):
    ...
    def __init__(self, in_features, out_features, s=30.0, m=0.50, easy_margin=False):
        ...
        self.m = m
        self.weight = Parameter(torch.FloatTensor(out_features, in_features))
        nn.init.xavier_uniform_(self.weight) # normal(self.weight) #
...
if __name__ == "__main__":
    ...
    print(torch.sum(tensor, dim=1).numpy())